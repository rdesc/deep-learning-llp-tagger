...
from slim_training_data import *
...
parser.add_argument('--doSlimming',action="store_true")
...
if args.doProcessing:
    ...
    if not (args.doCombination):
        df = pd.read_pickle(args.output_combo_path + "/combine_output.pkl")
        #df = pd.read_pickle(args.output_combo_path + "/signal_2019-09-17_08-43-30_081507792_raw_output.pkl")
        print(args.input_combo_path + "/combine_output.pkl")
    if (args.makePlots):
        print("Plotting...")
...
if args.doTraining:
    ...
    list_constit_lstm = [12, 100, 400]
    list_track_lstm = [12, 100, 400]
    list_mseg_lstm = [5, 50, 500]
    ...
    #num_constit_lstm = list_constit_lstm, num_track_lstm = list_track_lstm, num_mseg_lstm = list_mseg_lstm )
    #keras_loop(inputFile , num_constit_lstm = list_constit_lstm, num_track_lstm = list_track_lstm, num_mseg_lstm = list_mseg_lstm )
    keras_loop(inputFile) 
    
if args.doSlimming:
    frac = 0.2
    inputFile = args.input_training_path + 'processed_output.pkl'
    df = slim_training_data(args.input_training_path, frac)
    df.to_pickle(args.input_training_path + "processed_output_frac"+str(frac)+".pkl")
